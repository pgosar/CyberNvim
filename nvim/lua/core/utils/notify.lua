...
local function _rename()
	...
	vim.ui.input({ prompt = "rename to> ", default = param.oldName }, function(input)
		if input == nil then
		...
		vim.lsp.buf_request(0, "textDocument/rename", param, function(err, result, ctx, config)
			if not result or (not result.documentChanges and not result.changes) then
				vim.notify(
					string.format("could not perform rename: %s -> %s", param.oldName, param.newName),
					"error",
			...
			vim.notify(str, "info", {
				title = string.format("[LSP] rename: %s -> %s", param.oldName, param.newName),
				timeout = 2500,
			})
		end)
	end)
